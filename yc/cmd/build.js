...
if ( args.cloud_build_type ) {
    ...
    if ('daily' === args.cloud_build_type) {// 日常
        config[config.build_type].assetsPublicPath = `//assets-daily.test.youku.com/ku/ycsatar/${versionPath}`;
    } else {// 线上
        config[config.build_type].assetsPublicPath = `//g.alicdn.com/ku/ycsatar/${versionPath}`;
    }
    // 云构建类型
